...
{
    public class LeaveVersion2Controller : Controller
    {
        ...
        public ActionResult SettingsLeaveTypes()
        {
            try
            {
                var EIC = Session["EIC"].ToString();
        ...
        public ActionResult SettingsLeaveLedgerCodes()
        {
            try
            {
                var EIC = Session["EIC"].ToString();
        ...
        public ActionResult SettingsLeaveCreditsEarned()
        {
            try
            {
                var EIC = Session["EIC"].ToString();
        ...
        public ActionResult SettingsLeaveConversionMatrixWH()
        {
            try
            {
                var EIC = Session["EIC"].ToString();
        ...
        public ActionResult SettingsLeaveConversionMatrixDP()
        {
            try
            {
                var EIC = Session["EIC"].ToString();
        ...
        public ActionResult SettingsLeaveAllotments()
        {
            try
            {
                var EIC = Session["EIC"].ToString();
        ...
        public ActionResult SettingsLeaveAdministrators()
        {
            // 30May2018@1050
            try
            {
        ...
        public ActionResult SaveRestoredLeave(int recNo, DateTime dateFiled, DateTime periodFrom, DateTime periodTo)
        {
            try
            {
                ...
                if (HROfficer!=null && leave != null)
                {
                    var toRestore = new tleaveRestore
                    {
                        EIC = leave.EIC,
                    ...
                    db.tleaveRestores.Add(toRestore);
                    db.SaveChanges();
                }
        ...
        public ActionResult Restoration()
        {
	    ...
	    {
			// 22pr2018 2300hrs
		    var data = db.vLeaveApp2.Where(r => r.EIC == EIC && r.IsApproved==true).OrderByDescending(r => r.periodFrom)
			.Select(r=> new
		    {
			    r.LeaveDescription, 
				r.dateFiled,
        ...
        {   
            ...
            try
            {
                ...
                var userAccess =
                    db.tmodUserModuleActivities.SingleOrDefault(r => r.actionName.Equals(actionName) && r.EIC.Equals(EIC));
                if (userAccess == null) return RedirectToAction("Index", "Home");
            ...
            var list = 
                db.vLeaveApp2
                    .Select(r => new
                    {
                        ...
                        detalye = db.vLeaveApp2.Where(x => x.EIC == r.EIC && x.dateFiled == r.dateFiled && (x.IsRecommendedAtHR == null))
                    })
        ...
        public ActionResult MyLeave()
        {
            ...
            var leaveModuleAdmin = db.trefLeaveAdministrators.SingleOrDefault(r => r.EIC == EIC && r.IsActive == true && r.Role.Equals("ModuleAdmin"));
        ...
        public ActionResult Settings()
        {
            ...
            try
            {
                ...
                // check if one of the HR BWD Officer
                var HROfficer = db.trefLeaveAdministrators.SingleOrDefault(r => r.EIC.Equals(EIC) && r.IsActive && (r.Role.Equals("ModuleAdmin")));
                var SystemAdmin = db.trefLeaveAdministrators.SingleOrDefault(r => r.EIC.Equals(EIC) && r.IsActive && (r.Role.Equals("SystemAdmin")));

                if (HROfficer != null || SystemAdmin != null) return View();
            ...
            try
            {
                ...
                foreach (var EIC in EICs)
                {
                    ...
                    foreach (var DTR in DTRListForPost)
                    {
                        ...
                        var ApprovedLeaves =
                            db.vLeaveApp2.Where(r => r.IsApproved == true).Where(
                        ...
                        /***** RESTORED LEAVES (VL and SL only) *****/
                        var restoredLeaves = db.tleaveRestores
                            .Where(r => r.EIC == _EIC && (r.datePosted >= dtrDateBegin && r.datePosted <= dtrDateEnd) && (r.leaveID.Equals("40301") || r.leaveID.Equals("40302")))
                            .OrderBy(r => r.datePosted).ToList();

                        foreach (var leave in restoredLeaves)
                        {
                            var controlNo = leave.controlNo;
                            var numDays = (leave.periodTo - leave.periodFrom).Value.TotalDays + 1;
                            // log entry to ledger details
                            var ledgerDetail = new tLeaveAppLedger
                            {
                                EIC = _EIC, LedgerCode = "107", Value = Math.Round(numDays, 4), Referrence = controlNo, Timestamp = PetsaKaron
                            };
                            db.tLeaveAppLedgers.Add(ledgerDetail);
                            db.SaveChanges();

                            var newIdLedgerDetail = ledgerDetail.Id;
                            var vl = leave.leaveID.Equals("40301") ? numDays : 0;
                            var sl = leave.leaveID.Equals("40302") ? numDays : 0;
                            VLBalance += vl;
                            SLBalance += sl;
                            ...
                            db.tLeaveAppLedgerMasters.Add(new tLeaveAppLedgerMaster
                            {
                                ...
                                SLCreditBalance = Math.Round(SLBalance, 4), 
                                Reference = newIdLedgerDetail.ToString(),
                                Remark = "RESTORED LEAVE", 
                                Timestamp = PetsaKaron
                            });
                        } // end - foreach (var leave in restoredLeaves)
        ...
        public JsonResult History()
        {
            ...
            var EIC = Session["EIC"].ToString();
            var data = db.vLeaveApp2.Where(r => r.EIC == EIC).OrderByDescending(r => r.periodFrom).Take(100);
            return Json(data.ToList(), JsonRequestBehavior.AllowGet);
        }
        ...
        public double getCSCEquivalentDays(int? minutesTardyUndertime)
        {
            ...
            int daysTU = 0, hoursTU = 0, minutesTU = 0;
